...
obj_data* getObjects(uint8_t* const distant_obj_detected, float* const distant_obj_angle, float* const distant_obj_dist, obj_data* obj_list, uint8_t* const obj_list_sz, short cm_ping_range)
{
	...
	*distant_obj_dist = 0;
    for (angle = 0; angle <= 180; angle += 2)
    {
        ...
        //char txt_buffer[200];//FIXME
        //snprintf(txt_buffer, 200, "%d %d %d %.2f\r\n", angle, scan.IR_raw_val, angle, scan.sound_dist);
        ...
        if (right_edge && scan.sound_dist > cm_ping_range) //left edge detection
        {
            ...
            *distant_obj_detected = (*distant_obj_detected || *obj_list_sz - old_obj_list_sz == 0) ? total_obj_detected : 0;
            obj_angle = (angle + right_edge) * 0.5;
            *distant_obj_angle = moving_avg(*distant_obj_angle, total_obj_detected, obj_angle);
            *distant_obj_dist = moving_avg(*distant_obj_dist, total_obj_detected, data_buffer[data_buffer_sz / 2 - 1].dist);
